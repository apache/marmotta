...
/**
 * Select the left-hand operator, if it exists, or fallback to the right-hand operator.
 *
 * @param <Node> the node type used by the backend
 * <p/>
 * Author: Chris Beer <cabeer@stanford.edu>
 */
public class NullCoalescingSelector<Node> implements NodeSelector<Node> {

    private NodeSelector<Node> left;
    private NodeSelector<Node> right;

    public NullCoalescingSelector(NodeSelector<Node> left, NodeSelector<Node> right) {
        this.left = left;
        this.right = right;
    }
    ...
    public Collection<Node> select(final RDFBackend<Node> rdfBackend, final Node context, final List<Node> path, final Map<Node, List<Node>> resultPaths) {
        ...
        final Collection<Node> selection = left.select(rdfBackend, context, path, resultPaths);
    ...
    public String getName(NodeBackend<Node> nodeRDFBackend) {
        throw new UnsupportedOperationException("cannot use fallbacks in unnamed field definitions because the name is ambiguous");
    }
    ...
    public boolean equals(Object o) {
        ...
        NullCoalescingSelector that = (NullCoalescingSelector) o;
