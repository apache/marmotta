...
/**
 * A KiWiConnection offers methods for storing and retrieving KiWiTriples,
 * KiWiNodes, and KiWiNamespaces in the database. It wraps a JDBC connection
 * which will be committed on commit(), rolled back on rollback() and closed on
 * close();
 * <p/>
...
public class KiWiConnection implements AutoCloseable {
    ...
    /**
     * Cache literals by literal cache key
     * (LiteralCommons#createCacheKey(String,Locale,URI))
     */
     ...
     * require a JDBC connection. This allows more efficient implementations in
     * case the queries can be answered directly from the cache.
     */
    ...
    /**
     * Load a KiWiNamespace with the given prefix, or null if the namespace does
     * not exist. The method will first look in the node cache for cached nodes.
     * If no cache entry is found, it will run a database query
    ...
    /**
     * Load a KiWiNamespace with the given uri, or null if the namespace does
     * not exist. The method will first look in the node cache for cached nodes.
     * If no cache entry is found, it will run a database query
     * ("load.namespace_prefix").
     *
    ...
    /**
     * Store the namespace passed as argument in the database. The database
     * might enfore unique constraints and thus throw an exception in case the
     * prefix or URI is already used.
     *
     * @param namespace the namespace to store
     * @throws SQLException the prefix or URI is already used, or a database
     * error occurred
     */
    ...
    /**
     * Delete the namespace passed as argument from the database and from the
     * caches.
     *
    ...
    /**
     * Load a KiWiNode by database ID. The method will first look in the node
     * cache for cached nodes. If no cache entry is found, it will run a
     * database query ('load.node_by_id') on the NODES table and construct an
     * appropriate subclass instance of KiWiNode with the obtained values. The
     * result will be constructed based on the value of the NTYPE column as
     * follows:
     * <ul>
     * <li>'uri' - KiWiUriResource using the id and svalue (as URI) columns</li>
     * <li>'bnode' - KiWiAnonResource using the id and svalue (as AnonId)
     * columns</li>
     * <li>'string' - KiWiStringLiteral using the id, svalue (literal value),
     * lang (literal language) and ltype (literal type) columns</li>
     * <li>'int' - KiWiIntLiteral using the id, svalue (string value), ivalue
     * (integer value) and ltype (literal type) columns</li>
     * <li>'double' - KiWiDoubleLiteral using the id, svalue (string value),
     * dvalue (double value) and ltype (literal type) columns</li>
     * <li>'boolean' - KiWiBooleanLiteral using the id, svalue (string value),
     * bvalue (boolean value) and ltype (literal type) columns</li>
     * <li>'date' - KiWiDateLiteral using the id, svalue (string value), tvalue
     * (time value) and ltype (literal type) columns</li>
     * </ul>
     ...
     * @return an instance of a KiWiNode subclass representing the node with the
     * given database id; type depends on value of the ntype column
     */
    ...
    /**
     * Batch load the nodes with the given ids. This method aims to offer
     * performance improvements by reducing database roundtrips.
     *
    ...
    /**
     * Load a KiWiUriResource by URI. The method will first look in the node
     * cache for cached nodes. If no cache entry is found, it will run a
     * database query ('load.uri_by_uri') on the NODES table and construct a new
     * KiWiUriResource using the values of the id and svalue columns.
     * <p/>
     * When a node is loaded from the database, it will be added to the
     * different caches to speed up subsequent requests.
     *
     * @param uri the URI of the resource to load
     * @return the KiWiUriResource identified by the given URI or null if it
     * does not exist
     */
    ...
    /**
     * Load a KiWiAnonResource by anonymous ID. The method will first look in
     * the node cache for cached nodes. If no cache entry is found, it will run
     * a database query ('load.bnode_by_anonid') on the NODES table and
     * construct a new KiWiAnonResource using the values of the id and svalue
     * columns.
     * <p/>
     * When a node is loaded from the database, it will be added to the
     * different caches to speed up subsequent requests.
     ...
     * @return the KiWiAnonResource identified by the given internal ID or null
     ...
     * cache entry is found, it will run a database query ("load.literal_by_v")
     ...
     * @return a KiWiDateLiteral with the correct date, or null if it does not
     * exist
     * @throws SQLException
     */
    ...
    /**
     * Load a integer literal with the long value given as argument if it
     * exists. The method will first look in the node cache for cached nodes. If
     * no cache entry is found, it will run a database query
     * ("load.literal_by_iv") on the NODES table and construct a new KiWiLiteral
     * using the values of the literal columns (svalue, ivalue, ...). The type
     * of literal returned depends on the value of the ntype column.
     * <p/>
     * When a node is loaded from the database, it will be added to the
     * different caches to speed up subsequent requests.
     *
     * @param value the value of the integer literal to load
     * @return a KiWiIntLiteral with the correct value, or null if it does not
     * exist
     * @throws SQLException
     */
    ...
    /**
     * Load a double literal with the double value given as argument if it
     * exists. The method will first look in the node cache for cached nodes. If
     * no cache entry is found, it will run a database query
     * ("load.literal_by_dv") on the NODES table and construct a new KiWiLiteral
     * using the values of the literal columns (svalue, ivalue, ...). The type
     * of literal returned depends on the value of the ntype column.
     * <p/>
     * When a node is loaded from the database, it will be added to the
     ...
     *
     * @param value the value of the integer literal to load
     * @return a KiWiDoubleLiteral with the correct value, or null if it does
     * not exist
     * @throws SQLException
     */
    ...
    /**
     * Load a boolean literal with the boolean value given as argument if it
     * exists. The method will first look in the node cache for cached nodes. If
     * no cache entry is found, it will run a database query
     * ("load.literal_by_bv") on the NODES table and construct a new KiWiLiteral
     * using the values of the literal columns (svalue, ivalue, ...). The type
     * of literal returned depends on the value of the ntype column.
     * <p/>
     * When a node is loaded from the database, it will be added to the
     ...
     *
     * @param value the value of the integer literal to load
     * @return a KiWiBooleanLiteral with the correct value, or null if it does
     * not exist
     * @throws SQLException
     */
    ...
    /**
     * Store a new node in the database. The method will retrieve a new database
     * id for the node and update the passed object. Afterwards, the node data
     * will be inserted into the database using appropriate INSERT statements.
     * The caller must make sure the connection is committed and closed
     * properly.
     * <p/>
     * If the node already has an ID, the method will do nothing (assuming that
     * it is already persistent)
     *
     *
     * @param node
     * @throws SQLException
    ...
    /**
     * Store a triple in the database. This method assumes that all nodes used
     * by the triple are already persisted.
     *
     * @param triple the triple to store
     * @throws SQLException
     * @throws NullPointerException in case the subject, predicate, object or
     * context have not been persisted
     * @return true in case the update added a new triple to the database, false
     * in case the triple already existed
     */
    ...
    /**
     * Return the identifier of the triple with the given subject, predicate,
     * object and context, or null if this triple does not exist. Used for quick
     * existance checks of triples.
     *
     * @param subject
     * @param predicate
     * @param object
    ...
    /**
     * Mark the triple passed as argument as deleted, setting the "deleted" flag
     * to true and updating the timestamp value of "deletedAt".
     * <p/>
     * The triple remains in the database, because other entities might still
     * reference it (e.g. a version). Use the method cleanupTriples() to fully
     * remove all deleted triples without references.
     *
     * @param triple
     */
    ...
    /**
     * Mark all triples contained in the context passed as argument as deleted,
     * setting the "deleted" flag to true and updating the timestamp value of
     * "deletedAt".
     * <p/>
     * The triple remains in the database, because other entities might still
     * reference it (e.g. a version). Use the method cleanupTriples() to fully
     * remove all deleted triples without references.
     * <p/>
     * Warning: this method skips some concurrency and transaction safeguards
     * for performance and therefore should only be called if run in an isolated
     * transaction!
     *
    ...
    /**
     * Mark all triples contained in the triple store as deleted, setting the
     * "deleted" flag to true and updating the timestamp value of "deletedAt".
     * <p/>
     * The triple remains in the database, because other entities might still
     * reference it (e.g. a version). Use the method cleanupTriples() to fully
     * remove all deleted triples without references.
    ...
    /**
     * Mark the triple passed as argument as not deleted, setting the "deleted"
     * flag to false and clearing the timestamp value of "deletedAt".
     * <p/>
     * Note that this operation should only be called if the triple was deleted
    ...
    /**
     * Return a Sesame RepositoryResult of statements according to the query
     * pattern given in the arguments. Each of the parameters subject,
     * predicate, object and context may be null, indicating a wildcard query.
     * If the boolean parameter "inferred" is set to true, the result will also
     * include inferred triples, if it is set to false only base triples.
     * <p/>
     * The RepositoryResult holds a direct connection to the database and needs
     * to be closed properly, or otherwise the system might run out of
     * resources. The returned RepositoryResult will try its best to clean up
     * when the iteration has completed or the garbage collector calls the
     * finalize() method, but this can take longer than necessary.
     *
     *
     ...
     * @param inferred if true, the result will also contain triples inferred by
     * the reasoner, if false not
     * @param wildcardContext if true, a null context will be interpreted as a
     * wildcard, if false, a null context will be interpreted as "no context"
     * @return a new RepositoryResult with a direct connection to the database;
     * the result should be properly closed by the caller
     */
    ...
    /**
     * Internal implementation for actually carrying out the query. Returns a
     * closable iteration that can be used in a repository result. The iteration
     * is forward-only and does not allow removing result rows.
     *
     * @param subject the subject to query for, or null for a wildcard query
     * @param predicate the predicate to query for, or null for a wildcard query
     * @param object the object to query for, or null for a wildcard query
     ...
     * @param wildcardContext if true, a null context will be interpreted as a
     * wildcard, if false, a null context will be interpreted as "no context"
     * @return a ClosableIteration that wraps the database ResultSet; needs to
     * be closed explicitly by the caller
     * @throws SQLException
     */
    ...
    /**
     * Construct an appropriate KiWiNode from the result of an SQL query. The
     * method will not change the ResultSet iterator, only read its values, so
     * it needs to be executed for each row separately.
     *
     * @param row
     * @return
     */
    ...
    /**
     * Construct a KiWiTriple from the result of an SQL query. The query result
     * is expected to contain the following columns:
     * <ul>
     * <li>id: the database id of the triple (long value)</li>
     * <li>subject: the database id of the subject (long value); the node will
     * be loaded using the loadNodeById method</li>
     * <li>predicate: the database id of the predicate (long value); the node
     * will be loaded using the loadNodeById method</li>
     * <li>object: the database id of the object (long value); the node will be
     * loaded using the loadNodeById method</li>
     * <li>context: the database id of the context (long value); the node will
     * be loaded using the loadNodeById method</li>
     * <li>creator: the database id of the creator (long value); the node will
     * be loaded using the loadNodeById method; may be null</li>
     * <li>deleted: a flag (boolean) indicating whether this triple has been
     * deleted</li>
     * <li>inferred: a flag (boolean) indicating whether this triple has been
     * inferred by the KiWi reasoner</li>
     * <li>createdAt: a timestamp representing the creation date of the
     * triple</li>
     * <li>createdAt: a timestamp representing the deletion date of the triple
     * (null in case triple is not deleted)</li>
     * </ul>
     * The method will not change the ResultSet iterator, only read its values,
     * so it needs to be executed for each row separately.
     *
...
//        result.setContext((KiWiResource) loadNodeById(row.getLong(5)));
    ...
    /**
     * Construct a batch of KiWiTriples from the result of an SQL query. This
     * query differs from constructTripleFromDatabase in that it does a
     * batch-prefetching for optimized performance
     *
     * @param row a database result containing the columns described above
     * @return a KiWiTriple representation of the database result
     */
    ...
    /**
     * Return the prepared statement with the given identifier; first looks in
     * the statement cache and if it does not exist there create a new
     * statement.
     *
     ...
     * Return the prepared statement with the given identifier; first looks in
     * the statement cache and if it does not exist there create a new
     * statement. This method is used for building statements with variable
     * argument numbers (e.g. in an IN).
     *
     * @param key the id of the statement in statements.properties
     * @return
     * @throws SQLException
     ...
     * (e.g. MySQL), this method will first update a sequence table and then
     * return the value.
     *
    ...
    /**
     * Return a collection of database tables contained in the database. This
     * query is used for checking whether the database needs to be created when
    ...
    /**
     * Return the metadata value with the given key; can be used by KiWi modules
     * to retrieve module-specific metadata.
     *
     * @param key
     * @return
     * @throws SQLException
    ...
    /**
     * Update the metadata value for the given key; can be used by KiWi modules
     * to set module-specific metadata.
     *
     * @param key
     * @return
     * @throws SQLException
     ...
     * This query is necessary for checking proper state of a database when
     * initialising the system.
     *
     * @return
     */
     ...
     * either the method <code>commit</code> or the method
     ...
     * setAutoCommit(true) is called while participating in a distributed
     * transaction, or this method is called on a closed connection
     * @see #getAutoCommit
     */
    ...
    /**
     * Return true if batched commits are enabled. Batched commits will try to
     * group database operations and keep a memory log while storing triples.
     * This can considerably improve the database performance.
     *
     * @return
     */
    ...
    /**
     * Enabled batched commits. Batched commits will try to group database
     * operations and keep a memory log while storing triples. This can
     * considerably improve the database performance.
     *
     * @return
     */
    ...
    /**
     * Return the size of a batch for batched commits. Batched commits will try
     * to group database operations and keep a memory log while storing triples.
     * This can considerably improve the database performance.
     *
     * @return
     */
    ...
    /**
     * Set the size of a batch for batched commits. Batched commits will try to
     * group database operations and keep a memory log while storing triples.
     * This can considerably improve the database performance.
     *
     * @param batchSize
     ...
     * method is called while participating in a distributed transaction, if
     * this method is called on a closed conection or this
     ...
     *
     * @exception java.sql.SQLException if a database access error occurs, this
     * method is called while participating in a distributed transaction, this
     * method is called on a closed connection or this <code>Connection</code>
     ...
     * <P>
     *
     * @exception java.sql.SQLException SQLException if a database access error
     * occurs
     */
    public void close() throws SQLException {
        closed = true;
    ...
    /**
     * A generic implementation of an SQL command that might fail (e.g. because
     * of a timeout or concurrency situation) and should be retried several
