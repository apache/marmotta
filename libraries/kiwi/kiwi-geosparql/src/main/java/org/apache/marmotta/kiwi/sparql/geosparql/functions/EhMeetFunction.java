...
public class EhMeetFunction implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_GEOSPARQL.EH_MEET.toString())) {
    ...
    public String getNative(KiWiDialect dialect, String... args) {
      if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                 ...
                 *   example: geof:ehMeet(?geom1, "POLYGON(( -7 43, -2 43, -2 38, -7 38, -7 43))"^^geo:wktLiteral)
                 * st_AsText condition: It is to use the geometry that is the result of another function geosparql.
                 *   example: geof:ehMeet(?geom1, geof:buffer(?geom2, 50, units:meter))
                 */
