...
public class EhDisjointFunction implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_GEOSPARQL.EH_DISJOINT.toString())) {
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                 ...
                 *   example: geof:ehDisjoint(?geom1, "POLYGON(( -7 43, -2 43, -2 38, -7 38, -7 43))"^^geo:wktLiteral)
                 * st_AsText condition: It is to use the geometry that is the result of another function geosparql.
                 *   example: geof:ehDisjoint(?geom1, geof:buffer(?geom2, 50, units:meter))
                 */
                ...
                return String.format("ST_Relate(%s, %s, 'FF*FF****')", geom1, geom2);
            }
        }
        throw new UnsupportedOperationException("ehDisjoint function not supported by dialect " + dialect);
    }
