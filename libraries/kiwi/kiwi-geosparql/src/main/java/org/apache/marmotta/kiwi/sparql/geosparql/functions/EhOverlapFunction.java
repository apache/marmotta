...
public class EhOverlapFunction implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_GEOSPARQL.EH_OVERLAP.toString())) {
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        ...
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                 ...
                 *   example: geof:ehOverlap(?geom, "POLYGON(( -7 43, -2 43, -2 38, -7 38, -7 43))"^^geo:wktLiteral)
                 * st_AsText condition: It is to use the geometry that is the result of another function geosparql.
                 *   example: geof:ehOverlap(?geom, geof:buffer(?geom2, 50, units:meter))
                 */
                ...
                return String.format("ST_Relate(%s, %s, 'T*T***T**')", geom1, geom2);
            }
        }
        throw new UnsupportedOperationException("ehOverlap function not supported by dialect " + dialect);
    }
