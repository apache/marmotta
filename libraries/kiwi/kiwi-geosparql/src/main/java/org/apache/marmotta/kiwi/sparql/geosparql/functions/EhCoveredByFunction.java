 ...
 * </ul>
 * CoveredBy Relate is calculated with "DE-9IM Intersection Pattern": defined in
...
public class EhCoveredByFunction implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_GEOSPARQL.EH_COVEREDBY.toString())) {
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                 ...
                 *   example: geof:ehCoveredBy(?geom1, "POLYGON(( -7 43, -2 43, -2 38, -7 38, -7 43))"^^geo:wktLiteral)
                 * st_AsText condition: It is to use the geometry that is the result of another function geosparql.
                 *   example: geof:ehCoveredBy(?geom1, geof:buffer(?geom2, 50, units:meter))
                 */
