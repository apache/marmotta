        ...
        if(dialect instanceof PostgreSQLDialect) {
            if(args.length == 2) {
                ...
                {  //If users insert Direct the WKT  Geometry 
                    return String.format("st_Crosses(substring( %s from position(' ' in %s) + 1 for char_length( %s ) ), %s )", args[0], args[0], args[0], args[1]);
                }        
                return String.format("st_Crosses(substring( %s from position(' ' in %s) + 1 for char_length( %s ) ), substring( %s from position(' ' in %s) + 1 for char_length( %s ) ) ) ", args[0], args[0], args[0], args[1], args[1], args[1]);
            } 
