...
/**
 * A SPARQL function for doing  touches analyzer  between two geometries. Should be implemented directly in
...
public class SfTouchesFunction implements NativeFunction {
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if(dialect instanceof PostgreSQLDialect) {
         ...
         //       { 
                return String.format("st_Touches(substring( %s from position(' ' in %s) + 1 for char_length( %s ) ), substring( %s from position(' ' in %s) + 1 for char_length( %s ) ) ) ", args[0], args[0], args[0], args[1], args[1], args[1]);
              //  }
            //    else
